...
class Tinfoil(
    ...
    suspend fun start(files: Map<String, UnifiedFile>, isSphaira: Boolean) = withContext(IoDispatcher) {
    ...
    private fun sendListOfFilesTinfoil(files: Map<String, UnifiedFile>): Boolean {
        ...
        val namesBytes = sb.toString().encodeToByteArray()
        
        // 1. Send Header (16 Bytes: 4-byte magic + 4-byte name-length + 8 bytes padding)
        val headerPacket = ByteArray(16)
        TUL0.copyInto(headerPacket, destinationOffset = 0)
        intToBytesLE(namesBytes.size).copyInto(headerPacket, destinationOffset = 4)
    ...
    private suspend fun handleFileRangeTinfoil(header: ByteArray, files: Map<String, UnifiedFile>): Boolean {
        ...
        TUC0.copyInto(responseHeader, destinationOffset = 0)
    ...
    private fun sendSphPacket(result: Int, arg3: Int, arg4: Int): Boolean {
        // 24 bytes: magic, result/cmd, arg3, arg4, arg5 (unused, stays zero), crc
        val packet = ByteArray(SPH_PACKET_SIZE)
        intToBytesLE(SPH_MAGIC).copyInto(packet, destinationOffset = 0)
        intToBytesLE(result).copyInto(packet, destinationOffset = 4) // arg2 (cmd/result)
        intToBytesLE(arg3).copyInto(packet, destinationOffset = 8)
        intToBytesLE(arg4).copyInto(packet, destinationOffset = 12)
        // bytes 16..19 (arg5) intentionally left zero

        val crc = Crc32c.calculate(packet, 0, 20)
        intToBytesLE(crc).copyInto(packet, destinationOffset = 20)
